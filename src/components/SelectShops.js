...
const SelectShops = (props) => {
    ...
    //路线下拉数据
    const [wareHouseTagLines, setWareHouseTagLines] = useState([
        { id: "", name: "全部线路" },
    ]);
    //页面中选中的Key
    ...
    const [modalSelectedRowKeys, setModalSelectedRowKeys] = useState(
        props.defaultValue && props.defaultValue.length
            ? props.defaultValue.map((item) => item.id)
            : []
    );
    //弹窗中选中的row
    ...
    const getWareHouseTagLines = async (wareHouseId) => {
        ...
        if (!wareHouseId)
            return setWareHouseTagLines([{ id: "", name: "全部线路" }]);
        const { result = [] } = await requestAPI({
            urlType: {
                type: "post",
    ...
    function deletePageSelectd(keys = []) {
        ...
        keys.forEach((key) => {
            setModalSelectedShops((modalSelectedShops) =>
                modalSelectedShops.filter((item) => item.id !== key)
            );
            setModalSelectedRowKeys((modalSelectedRowKeys) =>
                modalSelectedRowKeys.filter((item) => item !== key)
            );
        });
    }
    ...
    const handleMultipleSearch = async (searchStr) => {
        ...
        const modalSelectedShopsKeys = modalSelectedShops.map(
            (item) => item.id
        );
        let newarr = shopVoList.filter(
            (item) => !modalSelectedShopsKeys.includes(item.id)
        );
        setModalSelectedShops(modalSelectedShops.concat(newarr));
        setModalSelectedRowKeys(
            modalSelectedRowKeys.concat(newarr.map((item) => item.id))
        );
        ...
        <div>
            ...
            >
                {pageSelectedShopsKeys.length > 0 && (
                    <span>
                        ...
                        <Popconfirm
                            ...
                            placement="left"
                            onConfirm={() =>
                                deletePageSelectd(pageSelectedShopsKeys)
                            }
                        >
                            <Button
                                type="primary"
                ...
                <Button
                    ...
                    onClick={() => setSelectShopModalVisible(true)}
                >
                    选择门店
                </Button>
            </div>
                    ...
                    <FormItem>
                        {getFieldDecorator("multipleSearch", {
                            rules: [
                                {
                                    required: true,
                                    message: "请输入门店编号或名称，以“、”分隔",
                                },
                            ],
                        })(
                            <TextArea
                                rows={10}
                                placeholder="请输入门店编号或名称，以“、”分隔"
                            />
                        )}
                    </FormItem>
                </Modal>
