...
const MenuLayout = ({ router: { location }, children, dispatch, app }) => {
    ...
    const [collapsed, setCollapsed] = useState(
        !!window.sessionStorage.getItem("collapsed")
    );
        ...
        return menus.reduce((previousValue, currentValue) => {
            ...
            if (currentValue.children && currentValue.children.length) {
                Array.prototype.push.apply(
                    previousValue,
                    flatMenus(currentValue.children)
                );
            }
    ...
    function getBreadCrumbArray(currentLocation, breadArray) {
        ...
        if (currentLocation.menuPcode && currentLocation.menuPcode !== "001") {
            const parentMenu = flatedMenus.find(
                (item) => item.menuCode === currentLocation.menuPcode
            );
    ...
    const breadCrumbArray = useMemo(() => {
        ...
        window.document.title =
            (currentLocation && currentLocation.menuName) || "古茗电商";
        const breadArray = [];
    ...
    return (
        <Layout style={{ height: "100vh" }}>
            ...
            <Layout>
                ...
                <Content style={{ minHeight: "initial", padding: "0 24px" }}>
                    {children}
                </Content>
                <Footer
                    style={{
